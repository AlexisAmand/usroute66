...
<form name='gbookForm' action='<?php JRoute::_('index.php'); ?>' target='_top' method='post'>
	...
	<?php } ?>
   	<tr>
		...
		<td valign='top'><textarea style='width:245px;' rows='15' cols='50' name='gbtext' id='gbtext' class='inputbox'><?php echo $this->entry->gbtext; ?></textarea></td>
	</tr>
    <?php if ($this->params->get('enable_spam', true) AND ($this->params->get('enable_spam_reg') OR !$this->user->gid)) 
	{ ?>
		<tr>
			<td width='130'><label for='easycalccheck'><br /><?php echo JText::_('SPAM'); ?></label><span class='small'>*</span></td>
			<td><br /><?php echo $this->session->get('spamcheck1', null, 'easybookreloaded'); ?> + <?php echo $this->session->get('spamcheck2', null, 'easybookreloaded'); ?> = <input type="text" name="easycalccheck" id='easycalccheck' size="3" value="" /></td>
		</tr>
	<?php } ?>
    <tr>
