...
function style_vignettes() {
	if (navigator.userAgent.search(/MSIE [1-7]/) != -1) {
		...
		if (tb) {
			...
			var bc = (document.getElementById('vignettes_cat')) ? '#494949' : '#C0C0C0';
		...
		if (has) {
			style_change_thumb(has, '#C0C0C0');
		}
	}
}

// Vignettes en mode 'compact'.
function style_change_thumb(obj, bordercolor) {
	...
	if (cl && cl.search(/v_thumb/gi) != -1) {
		...
		obj.onmouseout = function() {
			var bc = (this.className.search(/v_recent/gi) != -1) ? '#BDCF7F' : bordercolor;
